...
""" Here we are testing and demonstrating the training set we wrote in writing.py using pygame and numpy to do the main work. """


# Check command-line arguments
if len(sys.argv) != 2:
    
    sys.exit("Usage: python detection.py model")
...
while True:
    ...
    if classification is not None:
        classificationText = largeFont.render(labels[classification], True, WHITE)
        ...
        # Show / Render time taken
        if prediction_time is not None:
            timeText = smallFont.render(f"{prediction_time:.2f} ms", True, WHITE)
